...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query("SELECT COUNT(c.idReportedRebel) FROM Report c WHERE c.idReportedRebel = ?1")
    Integer getRebelReported(Long idReported);

    @Query("SELECT COUNT(c.idReporterRebel) FROM Report c WHERE c.idReporterRebel = ?1 AND c.idReportedRebel = ?2")
